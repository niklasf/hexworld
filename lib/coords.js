...
define(function() {
    ...
    function hex (hex) {
        ...
        if (column % 3 == 1 || column % 3 == 2) {
            var x = Math.floor(column / 3);
            var y = Math.floor((row - (x % 2)) / 2);
        } else {
            var left = hex.left % 18;
            var top = hex.top % 36;

            if (Math.floor(hex.left / 54) % 2 == 1) {
                if (row % 2 == 0) {
                    if (left * 2 < top) {
                        var x = Math.floor(hex.left / 54) - 1;
                        var y = Math.floor(row / 2);
                    } else {
                        var x = Math.floor(hex.left / 54);
                        var y = Math.floor(row / 2) - 1;
                    }
                } else {
                    if (left * 2 < 36 - top) {
                        var x = Math.floor(hex.left / 54) - 1;
                ...
                if (row % 2 == 0) {
                    ...
                    console.log({leftx: left, topx: top});
                    if (left * 2 < top) {
                        console.log("This should be the one!");
                        var x = Math.floor(hex.left / 54) - 1;
                        var y = Math.floor(row / 2);
                    } else {
                        console.log("It's coming from herw!");
                        var x = Math.floor(hex.left / 54);
                        var y = Math.floor(row / 2);
                    }
                }
